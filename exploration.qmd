...
pal <- colorNumeric(
  palette = "YlGnBu",
  domain = dat$oil_production
)

leaflet(dat) %>% 
    addTiles()+
    fillColor = ~pal(oil_production)


    leaflet(dat) %>%
  addPolygons(stroke = FALSE, smoothFactor = 0.2, fillOpacity = 1)

binpal <- colorBin("Blues", dat$population, 6, pretty = FALSE)

pal <- colorBin("YlOrRd", domain = dat$oil_production)
...
map <- my_leafdown$draw_leafdown(
  fillColor = ~ colorNumeric("Greens", oil_production)(oil_production)
) 




colnames(dat_2021)[1] <- "iso-a3"
str_to_upper(dat_2021$`iso-a3`)


hcmap(
  map = "custom/world-highres3", # high resolution world map
  data = dat_2021, # name of dataset
  joinBy = "iso-a3",
...
dat_2021 <- st_as_sf(dat_2021, wkt = "country") 
 ...
 leaflet(dat_2021) %>%
  ...
  addPolylines(stroke = FALSE, smoothFactor = 0.3, fillOpacity = 1,
               fillColor = factor('oil_production'))
